'use client'
import {ArrowDownCircleIcon, ArrowUpCircleIcon, CheckCircleIcon as CheckedOutline} from "@heroicons/react/24/outline";
import {CheckCircleIcon as CheckedSolid} from "@heroicons/react/24/solid";
import {Button} from "@heroui/button";
import {Answer, Question, Vote} from "@/lib/types";
import {useState, useTransition} from "react";
import {acceptAnswer, addVote} from "@/lib/actions/questionActions";
    ...
    const handleAcceptAnswer = () => {
        ...
        startTransition(async () => {
            ...
            if (error) 
                ...
                successToast("Answer has been accepted.", 'Success');
        })
        ...
        <div className="shrink-0 flex flex-col gap-3 items-center justify-start mt-4">
            ...
            {isAnswer && (
                <Button
                    ...
                    isDisabled={target.accepted || askerId !
                        == currentUserId}
                    isLoading={pending && !targetBtn}
                    onPress={handleAcceptAnswer}
                >
                    {target.accepted ? (
                        <CheckedSolid className="text-success" />
                    ) : (
                        <CheckedOutline className="size-12 text-default-500" />
                    )}
                </Button>
            )}
        </div>
