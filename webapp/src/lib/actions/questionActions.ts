...
export async function getQuestionById(id: string): Promise<FetchResponse<Question>> {
   ...
   const ids = Array.from(userIds).sort();
    ...
    const {data: profiles, error: profileError} = await fetchClient<Profile[]>(
        profilesUrl,
        'GET',
        {cache: 'force-cache', next: {revalidate: 36000}});
    ...
    if (session){
        ...
        if (voteError)
            return {data: null, error: {message: 'Problem getting votes', status: 500}};
...
export async function postAnswer(data: AnswerSchema, questionId: string) {
    ...
    //Data is stale, update cached data
...
export async function editAnswer(questionId: string, answerId: string, data: AnswerSchema) {
